use crate::{
    kyro::Kyro,
    parser::tokens::{Literal, Token, TokenType},
};
...
impl<'a> Scanner<'a> {
    pub fn new(source: String, kyro: &'a mut Kyro) -> Self {
        ...
        match c {
            ...
            _ => {
                if Self::is_digit(c) {
                    ...
                    self.kyro.error(self.line, "Unexpected character.");
                }
            }
        }
    }
    ...
    fn string(&mut self) {
        ...
        if self.is_at_end() {
            self.kyro.error(self.line, "Unterminated string.");
            return;
        }
        self.advance();
